...
You can export all of your github account data on the [settings administration page](https://github.com/settings/admin). I got an email that it was ready 15 minutes after I requested it. It was 2.5 GB, so I won't be sharing it all in this repo, but I will include the .json files.

```{r}
list.files("data/github")
```

```{r}
repos <- read_json("data/github/repositories_000001.json", 
                   simplifyVector = T)
...
The scales aren't all the same. I'm going to use a hacky solution and add labels for January 1 and December 31 every year, but give them no label.
...
I couldn't set the colour of `geom_hline()` using `aes()` unless I was also setting `yintercept` in the mapping. I used the code from [Day -@sec-pictogram-colour] to change the ugly green to yellow.
